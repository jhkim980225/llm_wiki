...
- 중심 노드 = 이 개체, 이웃 = 1홉
- **그리는 이웃은 10개까지**(`MAX_GRAPH_NEIGHBORS`). 실측으로 사람 1명에 이웃이 198개
  달려서 전부 그리면 문서 안 패널이 읽히지 않는다. 문서 본문의 관계 목록은 자르지 않는다 —
  화면만 추린다
- 자를 때는 **관계 종류별로 돌아가며** 뽑는다. 앞에서부터 자르면 정렬 때문에 한 관계
  (`usesAccount` 등)가 10칸을 다 먹어 관계의 다양성이 안 보인다
- 툴바에 `이웃 198개 중 10`으로 몇 개를 감췄는지 알린다 — 조용히 자르지 않는다
